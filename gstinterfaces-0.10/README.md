...
GJS TypeScript type definitions for GstInterfaces-0.10 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.4.0.
...
| `@girs/gstinterfaces-0.10/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`gstinterfaces-0.10` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/gstinterfaces-0.10/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/gstinterfaces-0.10/vocabulary';
```

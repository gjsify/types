...
GJS TypeScript type definitions for GCalc-1 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.3.0.
...
| `@girs/gcalc-1/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`gcalc-1` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/gcalc-1/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/gcalc-1/vocabulary';
```

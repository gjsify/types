...
GJS TypeScript type definitions for Gly-2 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.3.0.
...
| `@girs/gly-2/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`gly-2` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/gly-2/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/gly-2/vocabulary';
```

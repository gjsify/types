...
GJS TypeScript type definitions for Template-1.0, generated from library version 3.40.0 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.3.0.
...
| `@girs/template-1.0/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`template-1.0` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/template-1.0/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/template-1.0/vocabulary';
```

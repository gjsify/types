...
GJS TypeScript type definitions for Snapd-1 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.4.0.
...
| `@girs/snapd-1/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`snapd-1` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/snapd-1/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/snapd-1/vocabulary';
```

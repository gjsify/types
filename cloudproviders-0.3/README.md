...
GJS TypeScript type definitions for CloudProviders-0.3 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.3.0.
...
| `@girs/cloudproviders-0.3/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`cloudproviders-0.3` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/cloudproviders-0.3/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/cloudproviders-0.3/vocabulary';
```

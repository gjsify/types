...
| `@girs/cmenu-3.0/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`cmenu-3.0` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/cmenu-3.0/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/cmenu-3.0/vocabulary';
```

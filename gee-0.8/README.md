...
GJS TypeScript type definitions for Gee-0.8 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.3.0.
...
| `@girs/gee-0.8/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`gee-0.8` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/gee-0.8/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/gee-0.8/vocabulary';
```

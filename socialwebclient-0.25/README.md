...
GJS TypeScript type definitions for SocialWebClient-0.25 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.4.0.
...
| `@girs/socialwebclient-0.25/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`socialwebclient-0.25` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/socialwebclient-0.25/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/socialwebclient-0.25/vocabulary';
```

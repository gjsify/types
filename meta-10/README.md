...
GJS TypeScript type definitions for Meta-10 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.4.0.
...
| `@girs/meta-10/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`meta-10` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/meta-10/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/meta-10/vocabulary';
```

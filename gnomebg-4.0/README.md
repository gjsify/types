...
GJS TypeScript type definitions for GnomeBG-4.0 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.3.0.
...
| `@girs/gnomebg-4.0/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`gnomebg-4.0` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/gnomebg-4.0/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/gnomebg-4.0/vocabulary';
```

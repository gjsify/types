/**
 * The GIR-derived widget VOCABULARY for Clutter-4.
 *
 * GENERATED — do not edit. Provenance: Clutter-4 — dropped empty base(s): Atk.ImplementorIface
 *
 * 66 instantiable GTypes (of which 0 concrete widgets), 80 declarations, 46 enum nick unions, 0 slot candidates.
 *
...
import type Clutter from './clutter-4.js';
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type ClutterInputDeviceConstructOnly = GObjectConstructOnly | 'backend' | 'device-manager' | 'device-mode' | 'device-node' | 'device-type' | 'has-cursor' | 'id' | 'n-mode-groups' | 'n-rings' | 'n-strips' | 'name' | 'product-id' | 'vendor-id';
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type ClutterVirtualInputDeviceConstructOnly = GObjectConstructOnly | 'device-manager' | 'device-type';

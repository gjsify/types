...
GJS TypeScript type definitions for Clutter-5 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.3.0.
...
| `@girs/clutter-5/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`clutter-5` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/clutter-5/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/clutter-5/vocabulary';
```

/**
 * The GIR-derived widget VOCABULARY for St-18.
 *
 * GENERATED — do not edit. Provenance: St-18 — dropped empty base(s): Atk.ImplementorIface
 *
 * 29 instantiable GTypes (of which 0 concrete widgets), 30 declarations, 12 enum nick unions, 0 slot candidates.
 *
...
import type St from './st-18.js';
import type { AtkComponentConstructOnly, AtkComponentProps, AtkGObjectAccessibleConstructOnly, AtkGObjectAccessibleProps, AtkObjectConstructOnly, AtkObjectProps, AtkValueConstructOnly, AtkValueProps } from '@girs/atk-1.0/vocabulary';
import type { ClutterActionConstructOnly, ClutterActionProps, ClutterActorAccessibleConstructOnly, ClutterActorAccessibleProps, ClutterActorConstructOnly, ClutterActorMetaConstructOnly, ClutterActorMetaProps, ClutterActorProps, ClutterAnimatableConstructOnly, ClutterAnimatableProps, ClutterContentConstructOnly, ClutterContentProps, ClutterEffectConstructOnly, ClutterEffectProps, ClutterGestureConstructOnly, ClutterGestureProps, ClutterInputContentPurposeNick, ClutterOffscreenEffectConstructOnly, ClutterOffscreenEffectProps, ClutterOrientationNick, ClutterShaderEffectConstructOnly, ClutterShaderEffectProps } from '@girs/clutter-18/vocabulary';
...
export type StSystemAccentColorNick = 'blue' | 'teal' | 'green' | 'yellow' | 'orange' | 'red' | 'pink' | 'purple' | 'slate';
...
export interface StBinProps extends StWidgetProps, ClutterAnimatableProps {
    /** The child #ClutterActor of the #StBin container. */
    child?: Clutter.Actor;
}
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StBinConstructOnly = StWidgetConstructOnly | ClutterAnimatableConstructOnly;
...
export interface StBoxLayoutProps extends StViewportProps, ClutterAnimatableProps, StScrollableProps {
    /**
     * The orientation of the #StBoxLayout, either horizontal or vertical
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StBoxLayoutConstructOnly = StViewportConstructOnly | ClutterAnimatableConstructOnly | StScrollableConstructOnly;

/** Button widget A button widget with support for either a text label or icon, toggle mode and transitions effects between states. */
export interface StButtonProps extends StBinProps, ClutterAnimatableProps {
    /**
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StButtonConstructOnly = StBinConstructOnly | ClutterAnimatableConstructOnly;

/** A simple representation of the clipboard #StCliboard is a very simple object representation of the clipboard available to applications. */
export interface StClipboardProps extends GObjectProps {
}
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StClipboardConstructOnly = GObjectConstructOnly;

export interface StDndStartGestureProps extends ClutterGestureProps {
    /** @default FALSE */
    'manual-mode'?: boolean;
    /** @default 0 */
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StDndStartGestureConstructOnly = ClutterGestureConstructOnly;

/** A dynamically-sized Cairo drawing area #StDrawingArea allows drawing via Cairo; the primary difference is that it is dynamically sized. */
export interface StDrawingAreaProps extends StWidgetProps, ClutterAnimatableProps {
}
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StDrawingAreaConstructOnly = StWidgetConstructOnly | ClutterAnimatableConstructOnly;

/** Widget for displaying text #StEntry is a simple widget for displaying text. */
export interface StEntryProps extends StWidgetProps, ClutterAnimatableProps {
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StEntryConstructOnly = StWidgetConstructOnly | ClutterAnimatableConstructOnly;

/** Keyboard focus management #StFocusManager handles keyboard focus for all actors on the stage. */
export interface StFocusManagerProps extends GObjectProps {
}
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StFocusManagerConstructOnly = GObjectConstructOnly;

/** An accessible class with signals for implementing specific Atk interfaces #StGenericAccessible is mainly a workaround for the current lack of of a proper support for GValue at javascript. */
export interface StGenericAccessibleProps extends StWidgetAccessibleProps, AtkComponentProps, AtkValueProps {
}
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StGenericAccessibleConstructOnly = StWidgetAccessibleConstructOnly | AtkComponentConstructOnly | AtkValueConstructOnly;

/** A simple styled icon actor #StIcon is a simple styled texture actor that displays an image from a stylesheet. */
export interface StIconProps extends StWidgetProps, ClutterAnimatableProps {
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StIconConstructOnly = StWidgetConstructOnly | ClutterAnimatableConstructOnly;
...
/** Looking up icons by name #StIconTheme provides a facility for looking up icons by name and size. */
export interface StIconThemeProps extends GObjectProps {
}
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StIconThemeConstructOnly = GObjectConstructOnly;

export interface StImageContentProps extends GObjectProps, ClutterContentProps, GIconProps, GLoadableIconProps {
    /** @default -1 */
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StImageContentConstructOnly = GObjectConstructOnly | ClutterContentConstructOnly | GIconConstructOnly | GLoadableIconConstructOnly | 'preferred-height' | 'preferred-width';

/** Widget for displaying text #StLabel is a simple widget for displaying text. */
export interface StLabelProps extends StWidgetProps, ClutterAnimatableProps {
    /**
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StLabelConstructOnly = StWidgetConstructOnly | ClutterAnimatableConstructOnly;

export interface StPasswordEntryProps extends StEntryProps, ClutterAnimatableProps {
    /**
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StPasswordEntryConstructOnly = StEntryConstructOnly | ClutterAnimatableConstructOnly;

/** A user interface element to control scrollable areas. */
export interface StScrollBarProps extends StWidgetProps, ClutterAnimatableProps {
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StScrollBarConstructOnly = StWidgetConstructOnly | ClutterAnimatableConstructOnly;

/** Container for scrollable children #StScrollView is a single child container for actors that implement #StScrollable. */
export interface StScrollViewProps extends StWidgetProps, ClutterAnimatableProps {
    /** The child #StScrollable of the #StScrollView container. */
    child?: St.Scrollable | null;
    /**
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StScrollViewConstructOnly = StWidgetConstructOnly | ClutterAnimatableConstructOnly;
...
/** A #ClutterActor that can be scrolled The #StScrollable interface is exposed by actors that support scrolling. */
...
export interface StSpinnerContentProps extends GObjectProps, ClutterContentProps {
}
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StSpinnerContentConstructOnly = GObjectConstructOnly | ClutterContentConstructOnly;
...
/** A set of stylesheets #StTheme holds a set of stylesheets. */
...
/** Holds global information about a tree of styled objects #StThemeContext is responsible for managing information global to a tree of styled objects, such as the set of stylesheets or the default font. */
...
/** Style information for one node in a tree of themed objects A #StThemeNode represents the CSS style information (the set of CSS properties) for one node in a tree of themed objects. */
export interface StThemeNodeProps extends GObjectProps {
}
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StThemeNodeConstructOnly = GObjectConstructOnly;

/** Scrollable container The #StViewport is a generic #StScrollable implementation. */
export interface StViewportProps extends StWidgetProps, ClutterAnimatableProps, StScrollableProps {
    /** @default TRUE */
    'clip-to-view'?: boolean;
}
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StViewportConstructOnly = StWidgetConstructOnly | ClutterAnimatableConstructOnly | StScrollableConstructOnly;

/** Base class for stylable actors #StWidget is a simple abstract class on top of #ClutterActor. */
export interface StWidgetProps extends ClutterActorProps, ClutterAnimatableProps {
    /**
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StWidgetConstructOnly = ClutterActorConstructOnly | ClutterAnimatableConstructOnly;

export interface StWidgetAccessibleProps extends ClutterActorAccessibleProps, AtkComponentProps {
}
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type StWidgetAccessibleConstructOnly = ClutterActorAccessibleConstructOnly | AtkComponentConstructOnly;

...
GJS TypeScript type definitions for Gcr-3, generated from library version 3.41.2 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.3.0.
...
| `@girs/gcr-3/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`gcr-3` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/gcr-3/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/gcr-3/vocabulary';
```

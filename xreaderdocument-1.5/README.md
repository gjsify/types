...
GJS TypeScript type definitions for XreaderDocument-1.5, generated from library version 4.6.4 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.4.0.
...
| `@girs/xreaderdocument-1.5/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`xreaderdocument-1.5` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/xreaderdocument-1.5/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/xreaderdocument-1.5/vocabulary';
```

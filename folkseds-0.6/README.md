...
GJS TypeScript type definitions for FolksEds-0.6 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.3.0.
...
| `@girs/folkseds-0.6/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`folkseds-0.6` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/folkseds-0.6/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/folkseds-0.6/vocabulary';
```

...
import './node-unique-3.0-import.d.ts';
...
interface App {
     ...
     */
    addCommand(commandName: string | null, commandId: number): void
    /**
    ...
    messageReceived(command: number, messageData: MessageData, time: number): Response

    // Own signals of Unique-3.0.Unique.App

    connect(sigName: "message-received", callback: App.MessageReceivedSignalCallback): number
    on(sigName: "message-received", callback: App.MessageReceivedSignalCallback, after?: boolean): NodeJS.EventEmitter
    once(sigName: "message-received", callback: App.MessageReceivedSignalCallback, after?: boolean): NodeJS.EventEmitter
    off(sigName: "message-received", callback: App.MessageReceivedSignalCallback): NodeJS.EventEmitter
...
class App extends GObject.Object {
     ...
     */
    constructor(name: string | null, startupId?: string | null) 
    /**
     * Creates a new #UniqueApp instance for `name` passing a start-up notification
     * id `startup_id`.  The name must be a unique identifier for the application,
     * and it must be in form of a domain name, like
     ...
     * @returns the newly created #UniqueApp instance.
     */
    static new(name: string | null, startupId?: string | null): App
...
interface Backend {
     ...
     */
    getWorkspace(): number
    // Has conflict: requestName(): boolean
    // Has conflict: sendMessage(commandId: number, messageData: MessageData, time: number): Response
    /**
     * FIXME
     * @param name FIXME
     */
...
interface MessageData {
     ...
     */
    setText(str: string | null, length: number): boolean
    /**

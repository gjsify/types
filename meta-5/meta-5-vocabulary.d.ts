/**
 * The GIR-derived widget VOCABULARY for Meta-5.
 *
 * GENERATED — do not edit. Provenance: Meta-5 — library 3.34.5 — dropped empty base(s): Atk.ImplementorIface
 *
 * 27 instantiable GTypes (of which 0 concrete widgets), 27 declarations, 28 enum nick unions, 0 slot candidates.
 *
...
import type Meta from './meta-5.js';
import type { ClutterActorConstructOnly, ClutterActorProps, ClutterAnimatableConstructOnly, ClutterAnimatableProps, ClutterContainerConstructOnly, ClutterContainerProps, ClutterContentConstructOnly, ClutterContentProps, ClutterGroupConstructOnly, ClutterGroupProps, ClutterScriptableConstructOnly, ClutterScriptableProps, ClutterStageConstructOnly, ClutterStageProps } from '@girs/clutter-5/vocabulary';
...
export type MetaCursorNick = 'none' | 'default' | 'north-resize' | 'south-resize' | 'west-resize' | 'east-resize' | 'se-resize' | 'sw-resize' | 'ne-resize' | 'nw-resize' | 'move-or-resize-window' | 'busy' | 'dnd-in-drag' | 'dnd-move' | 'dnd-copy' | 'dnd-unsupported-target' | 'pointing-hand' | 'crosshair' | 'ibeam' | 'last';
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type MetaCursorTrackerConstructOnly = GObjectConstructOnly;
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type MetaIdleMonitorConstructOnly = GObjectConstructOnly | 'device-id';
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type MetaRemoteAccessHandleConstructOnly = GObjectConstructOnly;

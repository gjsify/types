...
GJS TypeScript type definitions for CinnamonDesktop-3.0 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.4.0.
...
| `@girs/cinnamondesktop-3.0/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`cinnamondesktop-3.0` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/cinnamondesktop-3.0/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/cinnamondesktop-3.0/vocabulary';
```

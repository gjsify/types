...
GJS TypeScript type definitions for Vips-8.0, generated from library version 8.18.3 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.4.0.
...
| `@girs/vips-8.0/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`vips-8.0` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/vips-8.0/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/vips-8.0/vocabulary';
```

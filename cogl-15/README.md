...
GJS TypeScript type definitions for Cogl-15 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.4.0.
...
| `@girs/cogl-15/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`cogl-15` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/cogl-15/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/cogl-15/vocabulary';
```

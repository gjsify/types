...
| `@girs/gcalc-2/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`gcalc-2` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/gcalc-2/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/gcalc-2/vocabulary';
```

...
GJS TypeScript type definitions for Cogl-51 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.3.0.
...
| `@girs/cogl-51/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`cogl-51` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/cogl-51/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/cogl-51/vocabulary';
```

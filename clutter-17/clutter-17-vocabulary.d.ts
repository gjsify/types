/**
 * The GIR-derived widget VOCABULARY for Clutter-17.
 *
 * GENERATED — do not edit. Provenance: Clutter-17 — dropped empty base(s): Atk.ImplementorIface
 *
 * 59 instantiable GTypes (of which 0 concrete widgets), 72 declarations, 48 enum nick unions, 0 slot candidates.
 *
...
import type Clutter from './clutter-17.js';
...
import type { CoglShaderTypeNick } from '@girs/cogl-17/vocabulary';
...
export type ClutterTransferFunctionNick = 'srgb' | 'pq' | 'bt709' | 'linear';
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type ClutterInputDeviceConstructOnly = GObjectConstructOnly | 'bus-type' | 'capabilities' | 'device-mode' | 'device-node' | 'device-type' | 'has-cursor' | 'n-buttons' | 'n-dials' | 'n-mode-groups' | 'n-rings' | 'n-strips' | 'name' | 'product-id' | 'seat' | 'vendor-id';
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type ClutterSpriteConstructOnly = ClutterFocusConstructOnly | 'device' | 'sequence';

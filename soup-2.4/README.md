...
GJS TypeScript type definitions for Soup-2.4, generated from library version 2.74.3 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.3.0.
...
| `@girs/soup-2.4/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`soup-2.4` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/soup-2.4/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/soup-2.4/vocabulary';
```

...
GJS TypeScript type definitions for Gck-2, generated from library version 4.4.0 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.4.0.
...
| `@girs/gck-2/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`gck-2` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/gck-2/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/gck-2/vocabulary';
```

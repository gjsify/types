/**
 * The GIR-derived widget VOCABULARY for Gdk-3.0.
 *
 * GENERATED — do not edit. Provenance: Gdk-3.0 — library 3.24.53
 *
 * 10 instantiable GTypes (of which 0 concrete widgets), 10 declarations, 33 enum nick unions, 0 slot candidates.
 *
...
export type GdkVisibilityStateNick = 'unobscured' | 'partial' | 'fully-obscured';
export type GdkVisualTypeNick = 'static-gray' | 'grayscale' | 'static-color' | 'pseudo-color' | 'true-color' | 'direct-color';
export type GdkWindowEdgeNick = 'north-west' | 'north' | 'north-east' | 'west' | 'east' | 'south-west' | 'south' | 'south-east';
export type GdkWindowTypeNick = 'root' | 'toplevel' | 'child' | 'temp' | 'foreign' | 'offscreen' | 'subsurface';
...
export type GdkWindowWindowClassNick = 'input-output' | 'input-only';
...
/** GdkAppLaunchContext is an implementation of #GAppLaunchContext that handles launching an application in a graphical context. */
...
/** #GdkDisplay objects purpose are two fold: - To manage and provide information about input devices (pointers and keyboards) - To manage and provide information about the available #GdkScreens GdkDispl… */
...
export interface GdkDragContextProps extends GObjectProps {
}
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type GdkDragContextConstructOnly = GObjectConstructOnly;

/** #GdkDrawingContext is an object that represents the current drawing state of a #GdkWindow. */
export interface GdkDrawingContextProps extends GObjectProps {
    /**
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type GdkDrawingContextConstructOnly = GObjectConstructOnly | 'clip' | 'window';

/** A #GdkKeymap defines the translation from keyboard state (including a hardware key, a modifier mask, and active keyboard group) to a keyval. */
export interface GdkKeymapProps extends GObjectProps {
}
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type GdkKeymapConstructOnly = GObjectConstructOnly;
...
/** #GdkScreen objects are the GDK representation of the screen on which windows can be displayed and on which the pointer moves. */
export interface GdkScreenProps extends GObjectProps {
    'font-options'?: null;
    /** @default -1.000000 */
    resolution?: number;
}
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type GdkScreenConstructOnly = GObjectConstructOnly;

/** A #GdkVisual contains information about a particular visual. */
export interface GdkVisualProps extends GObjectProps {
}
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type GdkVisualConstructOnly = GObjectConstructOnly;

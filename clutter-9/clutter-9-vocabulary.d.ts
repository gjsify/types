/**
 * The GIR-derived widget VOCABULARY for Clutter-9.
 *
 * GENERATED — do not edit. Provenance: Clutter-9 — dropped empty base(s): Atk.ImplementorIface
 *
 * 60 instantiable GTypes (of which 0 concrete widgets), 73 declarations, 51 enum nick unions, 0 slot candidates.
 *
...
import type Clutter from './clutter-9.js';
...
/** The #ClutterBurNode structure is an opaque type whose members cannot be directly accessed. */
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type ClutterInputDeviceConstructOnly = GObjectConstructOnly | 'backend' | 'device-mode' | 'device-node' | 'device-type' | 'has-cursor' | 'n-buttons' | 'n-mode-groups' | 'n-rings' | 'n-strips' | 'name' | 'product-id' | 'seat' | 'vendor-id';
...
export interface ClutterStageProps extends ClutterActorProps, ClutterAnimatableProps, ClutterContainerProps, ClutterScriptableProps {
    /**
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type ClutterStageConstructOnly = ClutterActorConstructOnly | ClutterAnimatableConstructOnly | ClutterContainerConstructOnly | ClutterScriptableConstructOnly;

/**
 * The GIR-derived widget VOCABULARY for Clutter-11.
 *
 * GENERATED — do not edit. Provenance: Clutter-11 — dropped empty base(s): Atk.ImplementorIface
 *
 * 61 instantiable GTypes (of which 0 concrete widgets), 74 declarations, 52 enum nick unions, 0 slot candidates.
 *
...
/** A layout manager arranging children on a single line The #ClutterBoxLayout is a #ClutterLayoutManager implementing the following layout policy: - all children are arranged on a single line - the axis… */
...
/** Content for 2D painting The #ClutterCanvas class is a #ClutterContent implementation that allows drawing using the Cairo API on a 2D surface. */
...
/** An interface for container actors #ClutterContainer is an interface implemented by [class@Actor], and it provides some common API for notifying when a child actor is added or removed, as well as the … */
...
/** Settable only at construction — a renderer must REBUILD, not patch. */
export type ClutterInputDeviceConstructOnly = GObjectConstructOnly | 'backend' | 'capabilities' | 'device-mode' | 'device-node' | 'device-type' | 'has-cursor' | 'n-buttons' | 'n-mode-groups' | 'n-rings' | 'n-strips' | 'name' | 'product-id' | 'seat' | 'vendor-id';
...
/** A constraint that follows a path #ClutterPathConstraint is a simple constraint that modifies the allocation of the [class@Actor] to which it has been applied using a [class@Path]. */
...
/** Loads a scene from UI definition data #ClutterScript is an object used for loading and building parts or a complete scenegraph from external definition data in forms of string buffers or files. */
...
/** Override the UI definition parsing The #ClutterScriptable interface exposes the UI definition parsing process to external classes. */
...
export interface ClutterTextProps extends Omit<ClutterActorProps, 'position'>, ClutterAnimatableProps, ClutterContainerProps, ClutterScriptableProps {
    ...
    /**
     * The current input cursor position.
     * @deprecated since 1.12: Use [property@Text:cursor-position] instead.
     */
    position?: number;

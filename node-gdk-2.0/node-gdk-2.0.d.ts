 ...
 * @param pixmapX X coordinate of location to place upper left corner of `pixmap`
 * @param pixmapY Y coordinate of location to place upper left corner of `pixmap`
 */
 ...
 * @param nPoints the size of the `points` array.
 */
 ...
 * @param pixbuf a #GdkPixbuf
 * @param srcX Source X coordinate within pixbuf.
 * @param srcY Source Y coordinates within pixbuf.
 * @param destX Destination X coordinate within drawable.
 * @param destY Destination Y coordinate within drawable.
 ...
 * @param xDither X offset for dither.
 * @param yDither Y offset for dither.
 */
 ...
 * @param nPoints the number of points to be drawn.
 */
 ...
 * @param nSegs the number of line segments to draw, i.e. the size of the   `segs` array.
 */
 ...
 * @param textLength the number of characters to draw.
 */
 ...
 * @param nTrapezoids the number of trapezoids to draw
 */
 ...
 * @param ownerEvents location to store boolean indicating whether   the `owner_events` flag to gdk_keyboard_grab() was %TRUE.
 ...
 * application.
 * @param time a timestamp from a #GdkEvent, or %GDK_CURRENT_TIME if no        timestamp is available.
 */
 ...
 * @param pixmapReturn Location to store a pointer to the created pixmap,   or %NULL if the pixmap is not needed.
 * @param maskReturn Location to store a pointer to the created mask,   or %NULL if the mask is not needed.
 * @param alphaThreshold Threshold value for opacity values.
 */
 ...
 * @param alphaThreshold Opacity values below this will be painted as zero; all other values will be painted as one.
 */
 ...
 * @param srcX Source X coordinate within pixbuf.
 * @param srcY Source Y coordinate within pixbuf.
 * @param destX Destination X coordinate within drawable.
 * @param destY Destination Y coordinate within drawable.
 * @param width Width of region to render, in pixels, or -1 to use pixbuf width
 * @param height Height of region to render, in pixels, or -1 to use pixbuf height
 ...
 * @param width Width of region to render, in pixels, or -1 to use pixbuf width.
 * @param height Height of region to render, in pixels, or -1 to use pixbuf height.
 * @param alphaMode Ignored. Present for backwards compatibility.
 * @param alphaThreshold Ignored. Present for backwards compatibility
 * @param dither Dithering mode for GdkRGB.
 * @param xDither X offset for dither.
 * @param yDither Y offset for dither.
 */
 ...
 * @param grabWindow location to store current grab window
 * @param ownerEvents location to store boolean indicating whether   the `owner_events` flag to gdk_pointer_grab() was %TRUE.
 ...
 * 
 * The array returned by this function should not be freed.
 * @param visualTypes return location for the available visual types
 ...
 * @param propType location to store the type of the property.
 * @param propFormat location to store the format of the property.
 ...
 * @param buttonPressrelease either %GDK_BUTTON_PRESS or %GDK_BUTTON_RELEASE
 ...
 * @param modifiers Keyboard modifiers the event is setup with.
 * @param keyPressrelease either %GDK_KEY_PRESS or %GDK_KEY_RELEASE
 ...
 */
function textPropertyToTextListForDisplay(display: Display, encoding: Atom, format: number, text: number, length: number, list: string | null): number
/**
 ...
 */
function wcstombs(src: WChar): string | null
/**
 ...
 */
function windowAtPointer(): [ /* returnType */ Window, /* winX */ number, /* winY */ number ]
/**
...
interface Colormap {
     ...
     * @param bestMatch If %TRUE, GDK will attempt to do matching against    existing colors if the color cannot be allocated as requested.
     * @returns %TRUE if the allocation succeeded.
     */
     ...
     * @param nColors The number of colors in `colors`.
     ...
     * @param bestMatch If %TRUE, GDK will attempt to do matching against    existing colors if the colors cannot be allocated as requested.
...
interface Display {
    ...
    // Has conflict: getNScreens(): number
    /**
     ...
     */
    getPointer(): [ /* screen */ Screen, /* x */ number, /* y */ number, /* mask */ ModifierType ]
    /**
     ...
     */
    getWindowAtPointer(): [ /* returnType */ Window, /* winX */ number, /* winY */ number ]
    /**
    ...
    on(sigName: "closed", callback: Display.ClosedSignalCallback, after?: boolean): NodeJS.EventEmitter
    once(sigName: "closed", callback: Display.ClosedSignalCallback, after?: boolean): NodeJS.EventEmitter
    off(sigName: "closed", callback: Display.ClosedSignalCallback): NodeJS.EventEmitter
...
interface DisplayManager {
    ...
    displayOpened(display: Display): void
    ...
    on(sigName: "display-opened", callback: DisplayManager.DisplayOpenedSignalCallback, after?: boolean): NodeJS.EventEmitter
    once(sigName: "display-opened", callback: DisplayManager.DisplayOpenedSignalCallback, after?: boolean): NodeJS.EventEmitter
    off(sigName: "display-opened", callback: DisplayManager.DisplayOpenedSignalCallback): NodeJS.EventEmitter
...
interface Drawable {
    ...
    createCairoSurface(width: number, height: number): cairo.Surface
    drawArc(gc: GC, filled: boolean, x: number, y: number, width: number, height: number, angle1: number, angle2: number): void
    drawDrawable(gc: GC, src: Drawable, xsrc: number, ysrc: number, xdest: number, ydest: number, width: number, height: number): void
    drawDrawableWithSrc(gc: GC, src: Drawable, xsrc: number, ysrc: number, xdest: number, ydest: number, width: number, height: number, originalSrc: Drawable): void
    drawGlyphs(gc: GC, font: Pango.Font, x: number, y: number, glyphs: Pango.GlyphString): void
    drawGlyphsTransformed(gc: GC, matrix: Pango.Matrix, font: Pango.Font, x: number, y: number, glyphs: Pango.GlyphString): void
    drawImage(gc: GC, image: Image, xsrc: number, ysrc: number, xdest: number, ydest: number, width: number, height: number): void
    drawLines(gc: GC, points: Point, npoints: number): void
    drawPixbuf(gc: GC, pixbuf: GdkPixbuf.Pixbuf, srcX: number, srcY: number, destX: number, destY: number, width: number, height: number, dither: RgbDither, xDither: number, yDither: number): void
    drawPoints(gc: GC, points: Point, npoints: number): void
    ...
    refCairoSurface(): cairo.Surface
    setCairoClip(cr: cairo.Context): void
    /**
     ...
     * @param cmap 
     */
    setColormap(cmap: Colormap): void
...
class Drawable extends GObject.Object {
...
interface GC {
     ...
     * @param srcGc the source graphics context.
     */
    copy(srcGc: GC): void
    // Has conflict: getValues(values: GCValues): void
    /**
    ...
    // Has conflict: setDashes(dashOffset: number, dashList: number, n: number): void
    /**
     ...
     * @param joinStyle the in which lines are joined together.
     */
    ...
    // Has conflict: setValues(values: GCValues, valuesMask: GCValuesMask): void
    /**
     ...
     * @virtual 
     * @param dashOffset the phase of the dash pattern.
     * @param dashList an array of dash lengths.
     ...
     * @param mask 
     */
    setValues(values: GCValues, mask: GCValuesMask): void
...
class GC extends GObject.Object {
     ...
     * @param valuesMask a bit mask indicating which fields in `values`   are set.
     * @returns the new graphics context.
     */
...
interface Keymap {
     ...
     */
    translateKeyboardState(hardwareKeycode: number, state: ModifierType, group: number): [ /* returnType */ boolean, /* keyval */ number, /* effectiveGroup */ number, /* level */ number, /* consumedModifiers */ ModifierType ]
    ...
    on(sigName: "direction-changed", callback: Keymap.DirectionChangedSignalCallback, after?: boolean): NodeJS.EventEmitter
    once(sigName: "direction-changed", callback: Keymap.DirectionChangedSignalCallback, after?: boolean): NodeJS.EventEmitter
    off(sigName: "direction-changed", callback: Keymap.DirectionChangedSignalCallback): NodeJS.EventEmitter
    ...
    on(sigName: "keys-changed", callback: Keymap.KeysChangedSignalCallback, after?: boolean): NodeJS.EventEmitter
    once(sigName: "keys-changed", callback: Keymap.KeysChangedSignalCallback, after?: boolean): NodeJS.EventEmitter
    off(sigName: "keys-changed", callback: Keymap.KeysChangedSignalCallback): NodeJS.EventEmitter
...
module Pixmap {
    ...
    interface ConstructorProperties extends Drawable.ConstructorProperties {
    }
...
class Pixmap extends Drawable {
     ...
     * @param transparentColor the color to be used for the pixels that are transparent in the input file. Can be %NULL, in which case a default color will be used.
     ...
     * @returns the #GdkPixmap.
     */
    static colormapCreateFromXpmD(drawable: Drawable, colormap: Colormap, mask: Bitmap, transparentColor: Color, data: string | null): Pixmap
    /**
     ...
     * @param drawable a #GdkDrawable, used to determine default values     for the new pixmap.
     * @param transparentColor This color will be used for the pixels     that are transparent in the input file. Can be %NULL     in which case a default color will be used.
     * @param data Pointer to a string containing the XPM data.
     * @returns the #GdkPixmap.
     */
    static createFromXpmD(drawable: Drawable, transparentColor: Color, data: string | null): [ /* returnType */ Pixmap, /* mask */ Bitmap ]
}
...
interface Screen {
     ...
     * @param monitorNum the monitor number, between 0 and gdk_screen_get_n_monitors (screen)
    ...
    on(sigName: "composited-changed", callback: Screen.CompositedChangedSignalCallback, after?: boolean): NodeJS.EventEmitter
    once(sigName: "composited-changed", callback: Screen.CompositedChangedSignalCallback, after?: boolean): NodeJS.EventEmitter
    off(sigName: "composited-changed", callback: Screen.CompositedChangedSignalCallback): NodeJS.EventEmitter
    ...
    on(sigName: "monitors-changed", callback: Screen.MonitorsChangedSignalCallback, after?: boolean): NodeJS.EventEmitter
    once(sigName: "monitors-changed", callback: Screen.MonitorsChangedSignalCallback, after?: boolean): NodeJS.EventEmitter
    off(sigName: "monitors-changed", callback: Screen.MonitorsChangedSignalCallback): NodeJS.EventEmitter
    emit(sigName: "monitors-changed", ...args: any[]): void
    connect(sigName: "size-changed", callback: Screen.SizeChangedSignalCallback): number
    on(sigName: "size-changed", callback: Screen.SizeChangedSignalCallback, after?: boolean): NodeJS.EventEmitter
    once(sigName: "size-changed", callback: Screen.SizeChangedSignalCallback, after?: boolean): NodeJS.EventEmitter
    off(sigName: "size-changed", callback: Screen.SizeChangedSignalCallback): NodeJS.EventEmitter
...
interface Visual {
     ...
     */
    getBluePixelDetails(): [ /* mask */ number, /* shift */ number, /* precision */ number ]
    /**
     ...
     * to how much precision the pixel value contains for a particular primary.
     */
    getGreenPixelDetails(): [ /* mask */ number, /* shift */ number, /* precision */ number ]
    /**
     ...
     * to how much precision the pixel value contains for a particular primary.
     */
    getRedPixelDetails(): [ /* mask */ number, /* shift */ number, /* precision */ number ]
    /**
...
module Window {
     ...
     */
    interface FromEmbedderSignalCallback {
        (embedderX: number, embedderY: number): void
    }

    /**
     * Signal callback interface for `pick-embedded-child`
     */
    interface PickEmbeddedChildSignalCallback {
     ...
     */
    interface ToEmbedderSignalCallback {
        (offscreenX: number, offscreenY: number): void
    }
...
interface Window {
     ...
     */
    coordsFromParent(parentX: number, parentY: number): [ /* x */ number, /* y */ number ]
    /**
     ...
     */
    coordsToParent(x: number, y: number): [ /* parentX */ number, /* parentY */ number ]
    /**
     ...
     */
    getPointer(): [ /* returnType */ Window, /* x */ number, /* y */ number, /* mask */ ModifierType ]
    /**
     ...
     * invalidated.
     * @param region a #GdkRegion
     * @param childFunc function to use to decide if to recurse to a child,              %NULL means never recurse.
     * @param userData data passed to `child_func`
     */
    invalidateMaybeRecurse(region: Region, childFunc: any | null, userData: any | null): void
    /**
     ...
     * is called.
     * @param drawable a #GdkDrawable
     * @param srcX x position in `window`
     * @param srcY y position in `window`
     * @param destX x position in `drawable`
     * @param destY y position in `drawable`
     ...
     * @param parentRelative whether the tiling origin is at the origin of   `window'`s parent
     */
     ...
     * @param minWidth minimum width hint
     * @param minHeight minimum height hint
     * @param maxWidth max width hint
     * @param maxHeight max height hint
     ...
     * @param iconWindow a #GdkWindow to use for the icon, or %NULL to unset
    ...
    on(sigName: "from-embedder", callback: Window.FromEmbedderSignalCallback, after?: boolean): NodeJS.EventEmitter
    once(sigName: "from-embedder", callback: Window.FromEmbedderSignalCallback, after?: boolean): NodeJS.EventEmitter
    off(sigName: "from-embedder", callback: Window.FromEmbedderSignalCallback): NodeJS.EventEmitter
    ...
    on(sigName: "pick-embedded-child", callback: Window.PickEmbeddedChildSignalCallback, after?: boolean): NodeJS.EventEmitter
    once(sigName: "pick-embedded-child", callback: Window.PickEmbeddedChildSignalCallback, after?: boolean): NodeJS.EventEmitter
    off(sigName: "pick-embedded-child", callback: Window.PickEmbeddedChildSignalCallback): NodeJS.EventEmitter
    ...
    on(sigName: "to-embedder", callback: Window.ToEmbedderSignalCallback, after?: boolean): NodeJS.EventEmitter
    once(sigName: "to-embedder", callback: Window.ToEmbedderSignalCallback, after?: boolean): NodeJS.EventEmitter
    off(sigName: "to-embedder", callback: Window.ToEmbedderSignalCallback): NodeJS.EventEmitter
...
interface DrawableClass {
    ...
    getSize: (drawable: Drawable) => [ /* width */ number, /* height */ number ]
...
interface PangoAttrEmbossColor {
...
interface PangoRendererPrivate {
}
...
interface WindowAttr {
    ...
    cursor: Cursor
    wmclassName: string | null
    wmclassClass: string | null

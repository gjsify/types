...
GJS TypeScript type definitions for Nautilus-4.1 using [ts-for-gir](https://github.com/gjsify/ts-for-gir) v5.3.0.
...
| `@girs/nautilus-4.1/vocabulary` | GIR-derived widget data: settable properties, enum nicks, slot candidates |
...
`nautilus-4.1` declares widgets, so it also carries what the GIR says about them, as
types and as values a test can read:

```ts
import type { Widgets, PropsOf } from '@girs/nautilus-4.1/vocabulary';
import { OWN_PROPS, ENUM_NICKS, PROVENANCE } from '@girs/nautilus-4.1/vocabulary';
```
